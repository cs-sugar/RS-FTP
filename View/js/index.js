...
window.onload = function ()
{
	...
	// AMFTP列表
	tr_arr = getElementByClassName('tr', 'tr');
	for (var k in tr_arr )
	{
		(function (obj)
		{
...
function all_select(o)
{
	for (var k in tr_arr )
	{
		...
		tr_arr[k].className = tr_arr[k].select.checked ? 'trmove' : '';
	}
}
...
//上传文件方法
function UpladFile() {
	var fileObj = G("file").files[0]; // js 获取文件对象
	...
	var url =  "./index.php?c=index&a=upload"; // 接收上传文件的后台地址
	...
	xhr.onload = function(evt) { //请求完成
		//服务断接收完文件返回的结果
		G("uploadicon").className = "IcoNormal";
		var nt = (new Date().getTime() - ot) / 1000;
		G("timedisplay").innerHTML = 'use:'+nt.toFixed(2)+'秒';
		G("deletebutton").style.display = 'none';
		G("_ReturnActionId").innerHTML = evt.target.responseText;
	};
	xhr.onerror = function(evt) { //请求失败
		G("_progressTarget").innerHTML = "";
		alert("上传失败！");
	};
	...
	xhr.upload.onloadstart = function(){//上传开始执行方法
		...
		G("_progressTarget").innerHTML = '<div id="SWFUpload_0_0" class="progressobj"><input type="button" class="IcoUpload" id="uploadicon"><span class="fle ftt">'+fileObj.name+' ('+filesize+')</span><span class="statebarSmallDiv"><span class="statebar" style="width: 0%;" id="progressBar">&nbsp;</span></span><span class="ftt" style="margin-top: 10px;" id="percentage">0%</span><span class="ftt" style="margin-top: 10px;" id="uploadspeed">0</span><span class="ftt" style="margin-top: 10px;" id="timedisplay">估计还需:0秒</span><a class="ftt a" onclick="cancleUploadFile()" id="deletebutton">删除</a></div>';
	};
...
function progressFunction(evt) {
	...
	var percentageDiv = G("percentage");
	...
	G("uploadspeed").innerHTML = speed+units;
	G("timedisplay").innerHTML = '估计还需:'+resttime+'秒';
	if(resttime==0)G("timedisplay").innerHTML = '正在保存中';
	if(bspeed==0) G("timedisplay").innerHTML = '';
}
...
var upload_file = function (obj, session_id)
{
	...
	upload_block.innerHTML = '<input type="file" id="file" name="myfile" style="display: none;"  onchange="UpladFile()" />';
	G('file').click();
	}else{
